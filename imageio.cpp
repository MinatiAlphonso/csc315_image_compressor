...
byte** readPNG( char* filename, unsigned& width, unsigned& height)
{
    ...
    // make width a multiple of 4, else grayscale image does not display 
    // properly in OpenGL.  Note: the openGL image display routines require
    // images to have 4-byte multiple widths
    unsigned pad = 0;
